/* program for 
1. overriding the SIGCHLD signal's handler 
2. a simple signal communication between child and process
*/
...
void sighandler_parent(int sig_num){
	printf("\n\nChild termination caught by the process with pid = %d",getpid());
}
...
int main(){
	...
	*/
	...
	if(pid > 0){
		...
		if(signal(SIGCHLD,sighandler_parent)==SIG_ERR){
			printf("\n\nError establishing handler for catching child process termination");
			return 2;
		}

		sleep(1); //to let the child establish the handler before parent send the signal
	...
	if(pid==0){
		...
		*/
		...
		sleep(4); //to let the child wait for the parent to send signal
